-- DistortionGL4WeaponsConfig.lua (SplinterFaction)
--
-- Weapon-related screen-space distortions for gfx_distortion_gl4.lua:
--   projectileDefDistortions[weaponDefID]  follows a projectile (heat haze on bolts/beams, missile thrust)
--   muzzleFlashDistortions[weaponDefID]    LIST spawned at Barrelfire (muzzle shockwave)
--   explosionDistortions[weaponDefID]      LIST spawned at Explosion (shockwave ring / heat bloom)
--   gibDistortion                          piece projectiles
--
-- Structure:
--   1. BaseClasses / SizeRadius / GetDistortionClass  - BAR's effect library, kept intact
--   2. AssignDistortionsToAllWeapons()                 - auto-assignment by weaponType.
--      SF changes: LaserCannon bolts get LaserBeamHeat scaled by thickness/damage;
--      customParams.heatweapon selects heat variants (BeamLaser -> HeatRayHeat,
--      others -> FireHeat explosion); two BAR bugs fixed (global leak, 4-arg AirBomb call).
--   3. SF post-pass honouring the expl_light_heat_* customParams SF weapondefs already carry
--      from the legacy light_effects heat pass, plus expl_noheatdistortion.
--   4. Name-keyed manual overrides.
--
...
local function AssignDistortionsToAllWeapons()
	for weaponID = 0, #WeaponDefs do
		...
		local t = {}
		local overrideTable = {}
		local distortionClass -- SF: was an accidental global in BAR
		...
		-- SF: Loz heat weapons are flagged with customParams.heatweapon (see game_heat_weapons.lua)
		local isHeatWeapon = (weaponDef.customParams and weaponDef.customParams.heatweapon) and true or false
		if weaponDef.type == "BeamLaser" then
			muzzleFlash = false

			if isHeatWeapon or string.find(weaponDef.name, "heat") then
		...
		elseif weaponDef.type == "LaserCannon" then
			...
			then
				...
				-- SF: heat shimmer along the bolt, scaled by visual thickness and damage
				local thickness = (weaponDef.visuals and weaponDef.visuals.thickness) or 2
				local powerFactor = math.min(1, damage / 400)
				local beamOverrides = {
					radius = 6 + thickness * 3,
					noiseStrength = 0.3 + 0.3 * powerFactor,
					effectStrength = 0.4 + 0.6 * powerFactor,
					noiseScaleSpace = 0.35,
				}
				projectileDefDistortions[weaponID] = GetDistortionClass("LaserBeamHeat", nil, beamOverrides)
				-- GetDistortionClass only applies overrides when a sizekey resolves; apply by hand
				for k, v in pairs(beamOverrides) do
					projectileDefDistortions[weaponID].distortionConfig[k] = v
				end
			end
		...
		elseif weaponDef.type == "AircraftBomb" then
			projectileDefDistortions[weaponID] =
				GetDistortionClass("AirBombProjectile", sizeclass, overrideTable)
		...
		if explosionDistortion then
			...
			elseif weaponDef.type == "Flame" or (isHeatWeapon and weaponDef.type ~= "BeamLaser") then
	...
	-- Spring.Echo("DistortionGL4 weapons conf using", usedclasses, "distortion types")
end
AssignDistortionsToAllWeapons() -- disable this if it doesn't work

--------------------------------------------------------------------------------
-- 3. SF post-pass: legacy heat-distortion customParams
--
--   expl_noheatdistortion          = true   no explosion distortion at all
--   expl_light_heat_radius         = elmos  absolute radius of the explosion distortion(s)
--   expl_light_heat_radius_mult    = x
--   expl_light_heat_life_mult      = x      lifeTime multiplier
--   expl_light_heat_strength_mult  = x      effectStrength / noiseStrength multiplier
--   muzzle_distortion_skip         = true
--   projectile_distortion_skip     = true
--------------------------------------------------------------------------------

local function privateCopyList(list)
	local out = {}
	for i, d in ipairs(list) do
		local c = deepcopy(d)
		c.distortionClassName = nil
		c.initComplete = nil
		c.distortionParamTable = nil
		out[i] = c
	end
	return out
end
...
local function applySFCustomParams()
	for weaponID = 0, #WeaponDefs do
		local cp = WeaponDefs[weaponID].customParams or {}
		if cp.expl_noheatdistortion then
			explosionDistortions[weaponID] = nil
		elseif explosionDistortions[weaponID] then
			local r = tonumber(cp.expl_light_heat_radius)
			local rm = tonumber(cp.expl_light_heat_radius_mult)
			local lm = tonumber(cp.expl_light_heat_life_mult)
			local sm = tonumber(cp.expl_light_heat_strength_mult)
			if r or rm or lm or sm then
				local list = privateCopyList(explosionDistortions[weaponID])
				for _, d in ipairs(list) do
					local dc = d.distortionConfig
					if r then dc.radius = r end
					if rm then dc.radius = (dc.radius or 0) * rm end
					if lm and dc.lifeTime and dc.lifeTime > 0 then dc.lifeTime = dc.lifeTime * lm end
					if sm then
						if dc.effectStrength then dc.effectStrength = dc.effectStrength * sm end
						if dc.noiseStrength then dc.noiseStrength = dc.noiseStrength * sm end
					end
				end
				explosionDistortions[weaponID] = list
			end
		end
		if cp.muzzle_distortion_skip then
			muzzleFlashDistortions[weaponID] = nil
		end
		if cp.projectile_distortion_skip then
			projectileDefDistortions[weaponID] = nil
		end
	end
end
applySFCustomParams()
...
--------------------------------------------------------------------------------
-- 4. SF manual overrides, keyed by weapondef name. explosion/muzzle take a LIST.
--   explosionDistortionsNames["lozmortar_shell"] = { GetDistortionClass("ExploShockWaveM", "Medium") }
--------------------------------------------------------------------------------
...
for name, list in pairs(explosionDistortionsNames) do
	if WeaponDefNames[name] then explosionDistortions[WeaponDefNames[name].id] = list end
end
for name, list in pairs(muzzleFlashDistortionsNames) do
	if WeaponDefNames[name] then muzzleFlashDistortions[WeaponDefNames[name].id] = list end
end
for name, params in pairs(projectileDefDistortionsNames) do
	if WeaponDefNames[name] then projectileDefDistortions[WeaponDefNames[name].id] = params end
end
