...
-- LuaShader helper (some engines expose it as gl.LuaShader, others require including it)
local LuaShader = gl.LuaShader
if not LuaShader then
	-- Common Spring/BAR path
	local ok, shaderLib = pcall(VFS.Include, "modules/graphics/LuaShader.lua")
	if ok then LuaShader = shaderLib end
end

if not LuaShader then
	-- No shader helper available; bail cleanly instead of crashing
	local function goodbye(reason)
		Spring.Echo("Map Grass GL4 widget exiting with reason: " .. reason)
		widgetHandler:RemoveWidget()
	end
	goodbye("LuaShader helper missing (gl.LuaShader is nil and modules/graphics/LuaShader.lua not found)")
	return
end
