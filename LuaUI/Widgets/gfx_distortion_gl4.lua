--------------------------------------------------------------------------------
-- Distortion GL4 (SplinterFaction port of BAR's gfx_distortion_gl4)
--
-- Screen-space heat shimmer / shockwave distortion. In DrawWorld (last layer):
-- copies the framebuffer, renders instanced distortion volumes into an offset
-- texture (depth-tested against the G-buffer), then resamples the screen copy
-- through the offsets. The whole pass is skipped when no distortion is live.
--
-- Inputs:
--   luaui/configs/DistortionGL4Config.lua         unit/feature/airjet distortions
--   luaui/configs/DistortionGL4WeaponsConfig.lua  projectile/muzzle/explosion/gib
--   gfx_deferred_rendering_gl4.lua                fans GadgetWeaponExplosion /
--       GadgetWeaponBarrelfire out to WG.distortionsgl4.VisibleExplosion/Barrelfire
--   api_unit_tracker_gl4.lua                      unit-attached lifecycle
--
-- Public API on WG.distortionsgl4: AddDistortion, RemoveDistortion,
-- GetDistortionVBO, IntensityMultiplier(v), RadiusMultiplier(v),
-- VisibleExplosion(...), Barrelfire(...).
--
...
-- SF: stubs for BAR-only globals
local BAR = BAR or {}
BAR.Debug = BAR.Debug or { TraceEcho = function() end, TraceFullEcho = function() end }
...
local ownsGadgetGlobals = false -- SF
...
function widget:Shutdown()
	...
	widgetHandler:RemoveAction("distortionintensity", "t")
	widgetHandler:RemoveAction("distortionradius", "t")
	-- TODO: delete the VBOs and shaders like a good boy
	-- SF:
	if WG.distortionsgl4 and WG.distortionsgl4.ownsGadgetGlobals then
		ownsGadgetGlobals = true
	end
	...
	if ownsGadgetGlobals then
		widgetHandler:DeregisterGlobal("GadgetWeaponExplosion")
		widgetHandler:DeregisterGlobal("GadgetWeaponBarrelfire")
		ownsGadgetGlobals = false
	end
	widgetHandler:DeregisterGlobal("AddDistortion")
	widgetHandler:DeregisterGlobal("RemoveDistortion")
	widgetHandler:DeregisterGlobal("GetDistortionVBO")

	deferredDistortionShader:Delete()
	if screenDistortionShader then -- SF: BAR leaked this
		screenDistortionShader:Delete()
	end
	if fullScreenQuadVAO and fullScreenQuadVAO.Delete then
...
function widget:Initialize()
	...
	widgetHandler:AddAction("distortionintensity", function(_, line)
		local v = tonumber(line)
		if v then
			intensityMultiplier = v
			spEcho("Distortion GL4: intensityMultiplier =", v)
		end
	end, nil, "t")
	widgetHandler:AddAction("distortionradius", function(_, line)
		local v = tonumber(line)
		if v then
			radiusMultiplier = v
			spEcho("Distortion GL4: radiusMultiplier =", v)
		end
	end, nil, "t")
	...
	-- SF: subscribe to the unit tracker
	if WG.unittrackerapi and WG.unittrackerapi.RegisterListener then
		WG.unittrackerapi.RegisterListener(widget)
		if WG.unittrackerapi.visibleUnits then
			widget:VisibleUnitsChanged(WG.unittrackerapi.visibleUnits, nil)
		end
	else
		spEcho("Distortion GL4: api_unit_tracker_gl4 not loaded, unit-attached distortions disabled")
	end
	...
	-- SF: the deferred lights widget owns the GadgetWeaponExplosion/Barrelfire
	-- globals and forwards to these. If lights are disabled, register them here.
	WG.distortionsgl4.VisibleExplosion = function(px, py, pz, weaponID, ownerID)
		widget:VisibleExplosion(px, py, pz, weaponID, ownerID)
	end
	WG.distortionsgl4.Barrelfire = function(px, py, pz, weaponID, ownerID)
		widget:Barrelfire(px, py, pz, weaponID, ownerID)
	end
	ownsGadgetGlobals = widgetHandler:RegisterGlobal("GadgetWeaponExplosion", WG.distortionsgl4.VisibleExplosion)
	if ownsGadgetGlobals then
		widgetHandler:RegisterGlobal("GadgetWeaponBarrelfire", WG.distortionsgl4.Barrelfire)
	end
