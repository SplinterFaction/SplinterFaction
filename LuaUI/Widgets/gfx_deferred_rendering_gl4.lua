--------------------------------------------------------------------------------
-- Deferred Lights GL4 (SplinterFaction port of BAR's gfx_deferred_rendering_GL4)
--
-- Renders instanced point/cone/beam light volumes against the engine G-buffer
-- in DrawWorld. Replaces the legacy immediate-mode pair
-- gfx_deferred_rendering.lua + gfx_light_effects.lua.
--
-- Inputs:
--   luaui/configs/DeferredLightsGL4config.lua         unit + feature lights
--   luaui/configs/DeferredLightsGL4WeaponsConfig.lua  projectile/muzzle/explosion/gib lights
--   LuaRules/Gadgets/gfx_explosion_lights.lua         forwards Explosion/Barrelfire
--       via Script.LuaUI.GadgetWeaponExplosion / GadgetWeaponBarrelfire
--   api_unit_tracker_gl4.lua                          unit-attached light lifecycle
--
-- Public API on WG.lightsgl4: AddPointLight, AddBeamLight, AddConeLight,
-- AddLight, RemoveLight, GetLightVBO, IntensityMultiplier(v),
-- RadiusMultiplier(v), ScreenSpaceShadows(n).
--
-- Requires AllowDeferredMapRendering=1 and AllowDeferredModelRendering=1
-- (set in LuaIntro/Addons/main.lua).
--
-- SF changes vs BAR are marked with "-- SF:".
--------------------------------------------------------------------------------
...
-- SF: stubs for BAR-only globals used by this file
local BAR = BAR or {}
BAR.Debug = BAR.Debug or {
	TraceEcho = function(...) end,
	TraceFullEcho = function(...) end,
}
if not (BAR.Utilities and BAR.Utilities.Gametype and BAR.Utilities.Gametype.IsSinglePlayer) then
	BAR.Utilities = BAR.Utilities or {}
	BAR.Utilities.Gametype = BAR.Utilities.Gametype or {}
	BAR.Utilities.Gametype.IsSinglePlayer = function()
		local humans = 0
...
-- SF: tracy profiler zones are a Recoil global; fall back to no-ops if absent
local tracy = tracy or { ZoneBeginN = function() end, ZoneEnd = function() end }

-- SF: direct includes; SF does not inject these into gl
...
local function AddRandomLight(which)
	local gf = gameFrame
...
function widget:Shutdown()
	...
	widgetHandler:RemoveAction("dlgl4intensity", "t")
	widgetHandler:RemoveAction("dlgl4radius", "t")
	WG.lightsgl4 = nil
	-- SF:
	if WG.unittrackerapi and WG.unittrackerapi.UnregisterListener then
	...
	-- hand the bridge back to the distortion widget if it is still running
	local d = WG.distortionsgl4
	if d and d.VisibleExplosion then
		widgetHandler:RegisterGlobal("GadgetWeaponExplosion", d.VisibleExplosion)
		widgetHandler:RegisterGlobal("GadgetWeaponBarrelfire", d.Barrelfire)
		d.ownsGadgetGlobals = true
	end
...
function widget:Initialize()
	...
	widgetHandler:AddAction("dlgl4intensity", function(_, line)
		local v = tonumber(line)
		if v then
			intensityMultiplier = v
			spEcho("Deferred Lights GL4: intensityMultiplier =", v)
		end
	end, nil, "t")
	widgetHandler:AddAction("dlgl4radius", function(_, line)
		local v = tonumber(line)
		if v then
			radiusMultiplier = v
			spEcho("Deferred Lights GL4: radiusMultiplier =", v)
		end
	end, nil, "t")
	...
	-- SF: subscribe to the unit tracker; it calls VisibleUnitsChanged on registration
	-- via the immediate sync below, then Added/Removed incrementally.
	if WG.unittrackerapi and WG.unittrackerapi.RegisterListener then
		WG.unittrackerapi.RegisterListener(widget)
		local vis = WG.unittrackerapi.visibleUnits
		if vis then
			widget:VisibleUnitsChanged(vis, nil)
		end
	else
		spEcho("Deferred Lights GL4: api_unit_tracker_gl4 not loaded, unit-attached lights disabled")
	end
	...
	-- SF: gfx_explosion_lights.lua gadget forwards these names
	-- If gfx_distortion_gl4 loaded first it may own these names; take them over
	-- so both widgets are fed from one registration.
	widgetHandler:DeregisterGlobal("GadgetWeaponExplosion")
	widgetHandler:DeregisterGlobal("GadgetWeaponBarrelfire")
	widgetHandler:RegisterGlobal("GadgetWeaponExplosion", function(px, py, pz, weaponID, ownerID)
		widget:VisibleExplosion(px, py, pz, weaponID, ownerID)
		local d = WG.distortionsgl4
		if d and d.VisibleExplosion then
			d.VisibleExplosion(px, py, pz, weaponID, ownerID)
		end
	end)
	widgetHandler:RegisterGlobal("GadgetWeaponBarrelfire", function(px, py, pz, weaponID, ownerID)
		widget:Barrelfire(px, py, pz, weaponID, ownerID)
		local d = WG.distortionsgl4
		if d and d.Barrelfire then
			d.Barrelfire(px, py, pz, weaponID, ownerID)
		end
	end)
