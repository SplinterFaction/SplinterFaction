...
function widget:GetInfo()
	return {
		name      = "How is my build - CSVBuilder",
		desc      = "Prints info about build timings and income to /MyBuildTimings and saves each match.",
...
local workingUnits = {}
local dataArray = {}
local logDir="MyBuildTimings/"
local file
local fileName

function widget:Initialize()
	Spring.CreateDir(logDir);
	fileName = logDir.. Game.mapName.. "_".. tostring(os.date("%m-%d-%Y_%H;%M")) ..".csv"
	file = io.open(fileName, "w");
	local header = string.format("%1s%1s%1s%1s%1s", "Unit Name,", "Start time,","Finish time,","Metal income,","Energy income")
	file:write(header.."\n")
	--file:write("------------------------------------------------------------------------------------------------------------\n")
	file:close()
end
...
function widget:UnitFinished(unitID, unitDefID, unitTeam)
	if unitTeam == myTeamID() then
		if workingUnits[unitID] ~= nil then
			...
			local body = string.format("%1s%1s%1s%1s%1s", UnitDefs[unitDefID].humanName..",", startTime..",",finishTime..",",startMI..",",startEI)
