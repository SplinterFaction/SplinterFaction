...
function init()
	...
	-- Hardcoded common resolutions from 720p to 4K
	local supportedResolutions = {
		'1280 x 720',   -- 720p
		'1600 x 900',   -- 900p
		'1920 x 1080',  -- 1080p (Full HD)
		'2560 x 1080',  -- Ultrawide 1080p
		'2560 x 1440',  -- 1440p (2K)
		'3440 x 1440',  -- Ultrawide 1440p
		'3840 x 2160',  -- 4K (UHD)
	}

	-- Build a lookup set so we don't add duplicates from infolog
	local resolutionSet = {}
	for _, res in ipairs(supportedResolutions) do
		resolutionSet[res] = true
	end

	-- Append any extra resolutions reported by the engine in infolog.txt
			...
			if addResolutions then
				...
				if resolution and string.len(resolution) >= 7 then
					...
					if tonumber(resolutionX) >= 640 and tonumber(resolutionY) >= 480 then
						if not resolutionSet[resolution] then
							supportedResolutions[#supportedResolutions+1] = resolution
							resolutionSet[resolution] = true
						end
					end
				else
					break
	...
	options = {
		...
		{id="grassdetail", group="gfx", name="Grass", type="slider", min=0, max=200, step=10, value=tonumber(Spring.GetConfigInt("GrassDetail",0) or 200), description='Amount of grass rendered\n\nChanges will be applied next game'},
		...
		{id="sndairabsorption", group="snd", name="Air absorption", type="slider", min=0, max=0.5, step=0.01, value=tonumber(Spring.GetConfigFloat("snd_airAbsorption",0.35) or 1),description="How much does the air absorb sound?"},
		...
		-- UI
		{id="buildordermenu", group="ui", name="Build Menu Preset", type="select", options={'Traditional Large','Traditional Small','Traditional Compact'}, value=Spring.GetConfigInt("evo_buildorderui",1) + 1, description='Use these presets to change the build menu size and position.\nBuild and Order menus can further be custom positioned using TweakMode (ctrl+f11)\n\nNOTE: Changing this setting will cause the interface to be reloaded'},
	...
	if getOptionByID('font') then
		...
		local fname = Spring.GetConfigString("ui_font", "Saira_SemiCondensed-SemiBold.ttf"):lower()
		...
		local fname = Spring.GetConfigString("ui_font2", "Saira_SemiCondensed-SemiBold.ttf"):lower()
