...
void main()
{
    ...
    if (UNIFORMLOC < 16u) {
        ...
        value = UNITUNIFORMS.userDefined[UNIFORMLOC >> 2u][UNIFORMLOC & 3u];
    } else {
        // health bar: hide while under construction and health merely tracks build progress
        float buildprogress = UNITUNIFORMS.userDefined[0].x; // -1.0 for fully built units, maintained by cus_gl4
        if (abs(buildprogress - value) < 0.03) {
            gl_Position = vec4(2.0, 2.0, 2.0, 1.0);
            g_color = vec4(0.0);
            g_uv = vec2(0.0);
            g_value = 0.0;
